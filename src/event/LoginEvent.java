...
import accountingGUI.accountingGUI;
...
//				System.out.println(role);
                // 登录成功，执行后续操作
                Leader.disappear();
                new LoginSuccessHandler().ManagerSuccess();
            } else {
                // 登录失败，显示错误消息
...
//	}
    ...
    public static class LoginSuccessHandler {
        public void ManagerSuccess() {
            ManagerGUI crudInterface = new ManagerGUI();
        ...
        public void AccountingSuccess() {
            accountingGUI crudInterface = new accountingGUI();
            crudInterface.setTitle("财务增删改查界面");
            crudInterface.setSize(1000, 500); // 调整窗口大小以适应组件
            crudInterface.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            crudInterface.setLocationRelativeTo(null);
